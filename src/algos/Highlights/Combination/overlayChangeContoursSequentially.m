...
addParameter(p,'BorderMargin',100);
...
borderMargin = p.Results.BorderMargin;
...
for i = 1:(numel(images) - 1)
    ...
    edgesRaw = edge(changeMask,'Canny');
    % 1) alles löschen, was bis 'borderMargin' an den Rand reicht
    if borderMargin > 0
        edgesRaw(1:borderMargin,:)                     = false;
        edgesRaw(end-borderMargin+1:end,:)             = false;
        edgesRaw(:,1:borderMargin)                     = false;
        edgesRaw(:,end-borderMargin+1:end)             = false;
    end

    % 2) Optionale Entfernung aller *Rest*-Objekte,
    %    die (trotzdem) Randkontakt hätten
    edgesRaw = imclearborder(edgesRaw);

    % 3) Erst jetzt verbreitern
    edges = imdilate(edgesRaw,strel('diamond',1));
    % --------------------------------------------------------------
